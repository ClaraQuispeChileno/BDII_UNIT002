...
function activate(context) {
    console.log('La extensión "database-documentation-system" está activa en el Panel (DB).');

    // Registrar proveedor de Webview para el panel inferior
    ...
    // Mantener comando original por retrocompatibilidad
    let disposable = vscode.commands.registerCommand('db-documenter.open', function (uri) {
        ...
        if (filePath && dbPanelProvider.view) {
            dbPanelProvider.view.show(true);
            dbPanelProvider.selectFileDirectly(filePath);
        } else {
            vscode.commands.executeCommand('workbench.view.extension.db-panel-container');
        }
    });
            ...
            try {
                ...
                if (mode === 'download') {
                    ...
                    if (saveUri) {
                        ...
                        vscode.window.showInformationMessage(`PDF de Auditoría guardado exitosamente en: ${path.basename(saveUri.fsPath)}`);
                    }
                } else {
                    // Mostrar reporte (abrir un Markdown temporal en VS Code)
    ...
    async handleConversion(filePath, targetFormat) {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                // Determinar lenguaje de visualización en VS Code
                let language = 'sql';
    ...
    async handleDataGeneration(filePath) {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                // Determinar el lenguaje y el contenido a mostrar
    ...
    getHtmlForWebview(webview) {
        ...
        <body>
            ...
            <script>
                ...
                let currentMenu = 'main'; // main, select_file, analyze, convert
                let selectedFile = null;
                let activeIndex = 0;
                let workspaceFilesList = [];

                // Definiciones de menú (Sin Emojis)
                const menus = {
                    main: () => [
                ...
                function runAnalysis(mode) {
                    ...
                    // Ocultar loading después de iniciar (VS Code maneja su propio loader de progreso)
                ...
                window.addEventListener('keydown', (e) => {
                    let itemsCount = 0;
                    if (currentMenu === 'select_file') {
                        itemsCount = workspaceFilesList.length + 2; // Buscar manual + archivos + Volver
                    } else {
                    ...
                    if (e.key === 'ArrowUp' || e.key === 'w' || e.key === 'k') {
                        ...
                        // Evitar seleccionar elementos deshabilitados
                ...
                // Escuchar mensajes del backend de VS Code
...
// Función generadora de documentación copiada de server.js para ejecución Offline/Local
